"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FhirService = void 0;
const common_1 = require("@nestjs/common");
const fhir_client_interface_1 = require("./interfaces/fhir-client.interface");
let FhirService = class FhirService {
    client;
    constructor(client) {
        this.client = client;
    }
    create(resourceType, resource) {
        return this.client.create(resourceType, resource);
    }
    read(resourceType, id) {
        return this.client.read(resourceType, id);
    }
    update(resourceType, id, resource) {
        return this.client.update(resourceType, id, resource);
    }
    delete(resourceType, id) {
        return this.client.delete(resourceType, id);
    }
    search(resourceType, params) {
        return this.client.search(resourceType, params);
    }
};
exports.FhirService = FhirService;
exports.FhirService = FhirService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(fhir_client_interface_1.FHIR_CLIENT)),
    __metadata("design:paramtypes", [Object])
], FhirService);
//# sourceMappingURL=fhir.service.js.map